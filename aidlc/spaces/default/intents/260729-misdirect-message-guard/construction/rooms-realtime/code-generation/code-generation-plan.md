...
### Step 7 — FR-10 참여자 관리 — **절단됨 (2026-07-30, Q1의 답)**

> **이 단계는 실행하지 않는다.** `scope-document.md`의 위험 등록부가 지정한 1순위 절단을 그대로 적용했다. U5(XL)에 시간을 돌려주기 위한 결정이며, U5가 일찍 끝나면 이 단계만 실행해 되돌릴 수 있다. 아래 체크박스는 되돌릴 때의 작업 목록으로 남긴다.
- [ ] `add_member()` — 친구 확인(BR-3.2), 중복 시 400 `ALREADY_MEMBER` (BR-10.1), `ai_check_enabled=TRUE` 초기화
- [ ] `leave_room()` — `room_members` 행 삭제. `member_user_ids()`가 실시간 조회라 브로드캐스트에서 자동으로 빠진다 (BR-10.2)
- [ ] 라우터 2개 + 테스트 5~6개
